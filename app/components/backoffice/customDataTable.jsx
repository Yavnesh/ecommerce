      ...
      <div className="p-8">
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <nav className="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4" aria-label="Table navigation">
                    <span className="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto pl-4">Showing <span className="font-semibold text-slate-800 dark:text-white">{itemStartIndex}-{itemEndIndex}</span> of <span className="font-semibold text-slate-800 dark:text-white">{data.length}</span></span>
                    <ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8 mb-2 pr-4">
                        <li>
                        ...
                        </li>
                        {
                            Array.from({length: totalPages}, (_ ,i)=>{
                                return(
                                    <li key={i}>
                                        <button onClick={()=> setCurrentPage(i + 1)} disabled={currentPage == i + 1} className={currentPage==i+1 ? "flex items-center justify-center px-3 h-8 leading-tight text-gray-50 bg-blue-600 border border-blue-300 hover:bg-blue-800 hover:text-white dark:bg-slate-300 dark:border-slate-400 dark:text-slate-800 dark:hover:bg-slate-300" : "flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"}>{i + 1}</button>
                                    </li>
                                );
                            })
                        }
